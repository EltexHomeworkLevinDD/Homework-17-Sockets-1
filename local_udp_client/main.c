...
#define SOCKET_PATH         "/tmp/sock_local_udp_client"
#define SERVER_SOCKET_PATH  "/tmp/sock_local_udp_server"
#define BUFFER_SIZE 32

int main(){
    unlink(SOCKET_PATH);
    // Создаём локальный потоковый сокет
    int own_fd = socket(AF_LOCAL, SOCK_DGRAM, 0);
    if (own_fd == -1){
    ...
    // Привязываем сокет клиента к адресу (Здесь биндить клиента обязательно)
